      ...
      <div className="border-b border-zinc-800 bg-zinc-900/30 px-6 py-4">
        <div className="max-w-xl relative">
          ...
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            className="w-full bg-zinc-800 border border-zinc-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-zinc-500 focus:outline-none focus:border-teal-500/50"
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {[{ icon: '📖', title: 'Quick Start Guide', desc: '5-minute setup and your first graph', badge: 'Recommended' }, { icon: '🎥', title: 'Video Tutorials', desc: '12 walkthrough videos', badge: '' }, { icon: '📡', title: 'API Reference', desc: 'Full REST API docs', badge: '' }, { icon: '💡', title: 'Best Practices', desc: 'Tips for optimal results', badge: '' }].map(c => (
                <div key={c.title} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 hover:border-zinc-700 cursor-pointer transition-colors flex items-center gap-3">
            ...
            {categories.filter(c => !searchQuery || c.name.toLowerCase().includes(searchQuery.toLowerCase()) || c.articles.some(a => a.title.toLowerCase().includes(searchQuery.toLowerCase()))).map(cat => (
              <div key={cat.name} className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
                ...
                {cat.articles.map((article, i) => (
                  <div key={i} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-800/50 cursor-pointer transition-colors border-b border-zinc-800/50 last:border-0">
          ...
          <div className="max-w-lg mx-auto space-y-4">
            <div className="grid grid-cols-2 gap-4">
              {[{ icon: MessageCircle, title: 'Live Chat', desc: 'Get help from our team', badge: 'Available', color: 'text-emerald-400 bg-emerald-400/10' }, { icon: Video, title: 'Video Call', desc: 'Schedule a 30-min demo', badge: 'Book slot', color: 'text-blue-400 bg-blue-400/10' }].map(c => (
                <div key={c.title} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 hover:border-zinc-700 cursor-pointer transition-colors">
                  ...
                  <span className="px-3 py-1 bg-zinc-800 text-zinc-300 text-xs rounded-lg">{c.badge}</span>
                </div>
              ))}
            </div>
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
              ...
              <div className="space-y-3">
                <input className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-zinc-500 focus:outline-none focus:border-teal-500/50" placeholder="Subject" />
                <textarea className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-zinc-500 focus:outline-none focus:border-teal-500/50 h-28 resize-none" placeholder="Describe your issue..." />
